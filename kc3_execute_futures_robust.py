...
# --- NO_TRADE_WINDOW_UTC ---
# Flat / in stables from 02:30 to 08:00 UTC daily.
# If a position is open while inside the window, we force-close it and remain flat.
NO_TRADE_START_MIN = 2*60 + 30   # 02:30 UTC
NO_TRADE_END_MIN   = 8*60 + 0    # 08:00 UTC

def _utc_min_of_day():
    now = datetime.now(timezone.utc)
    return now.hour*60 + now.minute

def in_no_trade_window_utc():
    m = _utc_min_of_day()
    return NO_TRADE_START_MIN <= m < NO_TRADE_END_MIN
...
def current_roi(symbol):
    ...
    # Prefer Binance-UI-style ROI for risk limits (hard stop), fallback to old lev_roi
    margin_roi = None
    try:
        margin_roi = pos.get('margin_roi') if isinstance(pos, dict) else None
    except Exception:
        margin_roi = None
    lev_roi_raw = pos.get('lev_roi') if isinstance(pos, dict) else None
    lev_roi_for_risk = margin_roi if (margin_roi is not None) else lev_roi_raw
...
def main():
    ...
    while True:
        ...
        # --- NO_TRADE_WINDOW_ENFORCE ---

        # Stay flat from 02:30–08:00 UTC.
            ...
            if sym and side:
                ...
                try:
                    ...
                    write_status({
                        ...
                        "note": "no_trade_window_force_close",

                        "window_utc": "02:30-08:00",
                    ...
                    try:
                        ...
                        write_status({"ts": datetime.now(timezone.utc).isoformat().replace("+00:00","Z"), "alive": True, "note": "no_trade_window_force_close_error", "err": repr(e)})
                ...
                try:
                    ...
                    write_status({"ts": datetime.now(timezone.utc).isoformat().replace("+00:00","Z"), "alive": True, "note": "no_trade_window_flat", "window_utc": "02:30-08:00"})
                    ...
                    try:
                        ...
                        if cfg.enabled and sym and side and isinstance(desired, dict):
                            ...
                            try:
                                ...
                                try:
                                    write_status({"ts": datetime.now(timezone.utc).isoformat().replace("+00:00","Z"), "alive": True, "note": "hard_max_lev_dd_check_error", "err": repr(e)})
                                except Exception:
                                    pass

                            # Rolling TP: arm once lev_roi >= 7%, then close if it retraces 30% from peak
                            try:
                                ARM = 0.07
                                DROP_FRAC = 0.30
                                ...
                                try:
                                    write_status({"ts": datetime.now(timezone.utc).isoformat().replace("+00:00","Z"), "alive": True, "note": "rolling_tp_check_error", "err": repr(e)})
                                except Exception:
                                    pass

                            do_stop, reason, details = kc3_edge_stop.should_edge_stop(state, cfg, z_now=z_now, lev_roi=lev_roi_for_risk)
