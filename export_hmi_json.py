#!/usr/bin/env python3
"""
export_hmi_json.py

Read the latest HMI value from output/fg2_daily.csv
and write it to hmi_latest.json at the repo root so the
website (index.html) can load it.

Expected CSV columns include:
- date
- FG_lite  (your HMI score)
"""

import json
from pathlib import Path

import pandas as pd

FG_CSV = Path("output/fg2_daily.csv")
OUT_JSON = Path("hmi_latest.json")


def main():
    if not FG_CSV.exists():
        raise SystemExit(f"CSV not found: {FG_CSV} (run compute_fg2_index.py first)")

    df = pd.read_csv(FG_CSV, parse_dates=["date"])
    if df.empty:
        raise SystemExit(f"CSV is empty: {FG_CSV}")

    df = df.sort_values("date")
    row = df.iloc[-1]

    hmi = float(row["FG_lite"])
    date_val = row["date"]

    # If parsed as Timestamp, format nicely; otherwise just str()
    try:
        date_str = pd.to_datetime(date_val).strftime("%Y-%m-%d")
    except Exception:
        date_str = str(date_val)

    payload = {
        "hmi": round(hmi, 1),
        "date": date_str,
    }

    OUT_JSON.write_text(json.dumps(payload, indent=2))
    print(f"Wrote {OUT_JSON} with HMI={payload['hmi']} on {payload['date']}")


if __name__ == "__main__":
    main()
