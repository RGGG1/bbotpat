...
HEADERS = {"User-Agent":"backtest-44x/1.1 (+bbot)"}
...
def _req(base, path, params):
    ...
    if r.status_code in (451,403,429):
        raise requests.HTTPError(f"{r.status_code} {r.reason} from {base}")
    r.raise_for_status()
    return r.json()
...
def binance_klines(symbol, interval, limit=1500, end_time_ms=None, start_time_ms=None, tries=6):
    ...
    for _ in range(tries):
        for b in BASES:
            try:
                return _req(b, "/api/v3/klines", params)
...
def fully_closed_daily(symbol):
    # up to yesterday 23:59:59.999 UTC so we only get fully-closed daily candles
    ...
    # filter from START_DATE (as aware datetime)
...
def run():
    try:
        log("Fetching fully-closed daily data…")
        daily={}
        for symbol,sym,fb in COINS:
            log(f"  • {sym} ({symbol})")
            rows=fully_closed_daily(symbol)
            if not rows:
                log(f"    !! No rows for {sym} — stopping.")
                return
            dts,closes=zip(*rows)
            daily[sym]={"symbol":symbol,"dates":list(dts),"closes":list(closes),"idx":{d:i for i,d in enumerate(dts)}}
            time.sleep(0.1)

        common=set(daily["BTC"]["dates"]) & set(daily["ETH"]["dates"]) & set(daily["SOL"]["dates"])
        cal=sorted(common)
        if not cal:
            log("!! No aligned calendar across coins."); return

        trades=[]; active=None; equity=1.0
        per_sym_hist={"BTC":[], "ETH":[], "SOL":[]}

        log("Running single-trade, compounded backtest…")
        for day in cal:
            # close active
            if active is not None:
                ...
                trades.append({
                    ...
                    "mae_pct":mae*100.0,"mfe_pct":mfe*100.0,"roi_pct":roi*100.0,"equity":equity,"reason":reason
                })
                per_sym_hist[sym].append({"mfe_pct":mfe*100.0})
                active=None

            # open at most ONE by BTC>ETH>SOL priority
            if active is None:
                candidates=[]
                for sym in ("BTC","ETH","SOL"):
                    ...
                    if abs(z) >= Z_THRESH:
                        ...
                        fb = next(fb for (_s1,_sym,fb) in COINS if _sym==sym)
                        tp_pct=adaptive_tp(sym, per_sym_hist[sym], fb)
        ...
        print(f"{'#':>3}  {'SYM':<3}  {'Dir':<5}  {'Entry UTC':<16}  {'Exit UTC':<16}  {'Hh':>4}  "
              f"{'Entry':>12}  {'Exit':>12}  {'MAE%':>7}  {'MFE%':>7}  {'ROI%':>7}  {'Equity×':>8}  {'Exit'}", flush=True)
        for i,tr in enumerate(trades,1):
            print(f"{i:>3}  {tr['sym']:<3}  {tr['direction']:<5}  "
                  ...
                  f"{tr['equity']:8.2f}  {tr['reason']}", flush=True)
        if not trades:
            log("No trades produced by the rule set (this would be unexpected).")
        else:
            log(f"\nTrades: {len(trades)}  |  Final equity: {trades[-1]['equity']:.2f}×")

    except Exception as e:
        log("ERROR while running backtest:")
        log(str(e))
        traceback.print_exc()
        sys.exit(1)
